...
1. Download Roadmap Epigenomics data and generate a directory structure of BED
   files containing the enhancer annotations
2. Download ENCODE data and prepare files for motif enrichment
3. Download the Thousand Genomes reference (for SHAPEIT2/IMPUTE2 imputation in
...
#+BEGIN_SRC sh
data/gwas-summary-stats/Makefile
#+END_SRC
...
#+BEGIN_SRC R
frea::plot_rrplot(gzfile('.../results/rrplot/core-features/all-traits/Enh.txt.gz'), 'SNP rank by p-value', 25000)
#+END_SRC
...
#+BEGIN_SRC sh
results/ld/pruned/Makefile
results/rrplot/core-features/pruned/Makefile
#+END_SRC

#+BEGIN_SRC R
frea::plot_rrplot(gzfile('.../results/rrplot/core-features/pruned/Enh.txt.gz'), 'Independent loci by p-value', 5000)
#+END_SRC
...
#+BEGIN_SRC sh
cd results/matched
make FEATURES=hb-features
make FEATURES=hb-features EnhClusters-significant.in
#+END_SRC

#+BEGIN_SRC R
data(honeybadger_cluster_density, pkg='frea')
frea::plot_enhancer_enrichments(gzfile('.../results/matched/hb-features/EnhClusters-significant.in'), honeybadger_cluster_density)
#+END_SRC
